...
cmd1 = "gunzip -c %s > deplex_temporary_files/%s" % (i1, I1)
subprocess.call(cmd1, shell = True)
cmd2 = "gunzip -c %s > deplex_temporary_files/%s" % (i2, I2)
subprocess.call(cmd2, shell = True)

cmd = "validate_mapping_file.py -m %s -o deplex_temporary_files/check_map" % ''.join(arguments['m'])
subprocess.call(cmd, shell = True)

cmd = "%sfix_mappingfile.py deplex_temporary_files/check_map/*_corrected.txt deplex_temporary_files" % scripts_dir
subprocess.call(cmd, shell = True)

## Demultiplexing and get quality filtering.
cmd1 = "split_libraries_fastq.py -o deplex_temporary_files/mapping_%s_1 -i %s -b deplex_temporary_files/%s   --rev_comp_mapping_barcodes -m deplex_temporary_files/*_corrected_1.txt  --store_demultiplexed_fastq" % (output_name, r1, I1)
cmd2 = "split_libraries_fastq.py -o deplex_temporary_files/mapping_%s_2 -i %s -b deplex_temporary_files/%s   --rev_comp_mapping_barcodes -m deplex_temporary_files/*_corrected_1.txt  --store_demultiplexed_fastq" % (output_name, r2, I1)
...
## Syncing
cmd = "%ssyncsort_fq deplex_temporary_files/mapping_%s_1/seqs.fastq deplex_temporary_files/mapping_%s_2/seqs.fastq deplex_temporary_files" % (scripts_dir, output_name, output_name)
subprocess.call(cmd, shell = True)
...
cmd = "cutadapt -a ATTAGATACCCTAGTAGTCC deplex_temporary_files/mapping_%s_1/seqs.fastq_paired.fq -o deplex_temporary_files/%s_only_R1_clean.fq -e 0.2" % (output_name, output_name)
subprocess.call(cmd, shell = True)
cmd = "cutadapt -a TTACCGCGGCTGCTGTCAC deplex_temporary_files/mapping_%s_2/seqs.fastq_paired.fq -o deplex_temporary_files/%s_only_R2_clean.fq -e 0.2" % (output_name, output_name)
subprocess.call(cmd, shell = True)

## Flash
cmd = "flash -m 10 -M 250 -r 250 -f 253 -t 12 -o deplex_temporary_files/%s deplex_temporary_files/%s_only_R1_clean.fq  deplex_temporary_files/%s_only_R2_clean.fq" % (output_name, output_name, output_name)
subprocess.call(cmd, shell = True)

## Syncing
cmd = "%ssyncsort_fq_readindex deplex_temporary_files/%s.extendedFrags.fastq deplex_temporary_files/%s deplex_temporary_files/%s" % (scripts_dir, output_name, I1, I2)
subprocess.call(cmd, shell = True)

## Splitting
print("In progress: split_libraries_fastq.py...")
cmd = "split_libraries_fastq.py -o deplex_temporary_files/mapping_%s -i deplex_temporary_files/%s.extendedFrags.fastq_synced.fq -b deplex_temporary_files/%s_synced.fq  --rev_comp_mapping_barcodes -m deplex_temporary_files/*_corrected_1.txt  --store_demultiplexed_fastq --phred_offset 33 -p 0.6" % (output_name, output_name, I1)
subprocess.call(cmd, shell = True)

cmd = "%ssplit_fastq.py deplex_temporary_files/mapping_%s/seqs.fastq deplex_temporary_files/%s_synced.fq" % (scripts_dir, output_name, I2)
...
for barcode in barcodes:
	...
	cmd = "%sfix_header.py deplex_temporary_files/check_map/*_corrected.txt deplex_temporary_files/mapping_all/seqs_%s.fna %s deplex_temporary_files" % (scirpts_dir, barcode, barcode)
	subprocess.call(cmd, shell = True)
	cmd = "%sfix_header_fastq.py deplex_temporary_files/check_map/*_corrected.txt deplex_temporary_files/mapping_all_fastq/seqs_%s.fastq %s deplex_temporary_files" % (scripts_dir, barcode, barcode)
	subprocess.call(cmd, shell = True)
...
subprocess.call(cmd, shell = True)
cmd = "%sfix_ID.py deplex_temporary_files/corrected_tmp.fna deplex_temporary_files" % scripts_dir
subprocess.call(cmd, shell = True)
cmd = "%sfix_ID_fastq.py deplex_temporary_files/corrected_tmp.fastq deplex_temporary_files" % scripts_dir
subprocess.call(cmd, shell = True)
...
subprocess.call(cmd, shell = True)
cmd = "mv deplex_temporary_files/%s.hist* %s_demultiplexed_data/Histograms/." % (output_name, output_name) 
subprocess.call(cmd, shell = True)
cmd = "mv deplex_temporary_files/check_map/*_corrected.txt %s_demultiplexed_data/." % output_name
subprocess.call(cmd, shell = True)
cmd = "mv deplex_temporary_files/corrected_all.* %s_demultiplexed_data/." % output_name
subprocess.call(cmd, shell = True)
